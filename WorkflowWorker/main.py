...
from shared.q_messaging_schemas.schemas import PROMPT_SCHEMA
...
class Worker:
    ...
    def dispatch_agent_task(self, workflow_id: str, task: dict, task_results: dict):
        ...
        try:
            ...
            # Use the shared schema for agent prompts
            buf = io.BytesIO()
            fastavro.writer(buf, PROMPT_SCHEMA, [payload])
            
            producer = self._client.create_producer(task_topic)
